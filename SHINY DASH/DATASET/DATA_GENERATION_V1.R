...
# Armazendado os dados
df <- data.frame(
  CITY = citys
)

# # Coordenadas Geográficas
# coord.citys <- data.frame(
#   CITY = c("BELÉM", "ANANINDEUA", "MARITUBA", "BENEVIDES", "SANTA BÁRBARA DO PARÁ"),
#   LAT = c(-1.4558, -1.3649, -1.3602, -1.3511, -1.2908),
#   LON = c(-48.4902, -48.3743, -48.3429, -48.2438, -48.2432)
# )
# 
# # Armazanando Dados Geográficos
# df <- df %>% 
#   left_join(coord.citys, by = "CITY")
...
df$CANAL_VENDA <- mapply(function(sex, city) {
  if (city %in% c("BENEVIDES", "SANTA BÁRBARA DO PARÁ")) {
...
df$CATEGORIA <- mapply(function(sex, city) {
  ...
  if (city == "BELÉM") {
    ...
    base_probs["ALIMENTOS"] <- base_probs["ALIMENTOS"] + 0.1
    base_probs["BRINQUEDOS"] <- base_probs["BRINQUEDOS"] + 0.1
  } else if (city %in% c("BENEVIDES", "SANTA BÁRBARA DO PARÁ")) {
    base_probs["ROUPAS"] <- base_probs["ROUPAS"] + 0.1
  }
...
df$RENDA_ESTIMADA <- mapply(function(cidade, idade, canal) {
  base <- case_when(
    ...
    cidade == "SANTA BÁRBARA DO PARÁ" ~ 1.7
  )
...
df$ESCOLARIDADE_SCORE <- mapply(function(idade, sexo, cidade) {
  base <- case_when(
    ...
    cidade == "SANTA BÁRBARA DO PARÁ" ~ 0.15
  )
